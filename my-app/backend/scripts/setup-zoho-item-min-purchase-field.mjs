/**
 * Create the Zoho Books *item* number custom field for min purchase count (if missing),
 * and set ZOHO_CUSTOM_FIELD_ITEM_MIN_PURCHASE_ID in backend/.env.
 *
 * Requires OAuth scopes including ZohoBooks.settings.CREATE.
 *
 * From my-app/backend:
 *   npm run zoho:setup-item-min-purchase-field
 */
...
const FIELD_LABEL = 'Min purchase qty'
const FIELD_API = 'cf_abhyati_min_purchase_qty'
...
async function main() {
  ...
  if (!field) {
    ...
    let r
    try {
      r = await createModule('/settings/fields', {
        label: FIELD_LABEL,
        data_type: 'number',
        entity: 'item',
        show_on_pdf: false,
      ...
      console.log('Number type failed; creating as string…')
      r = await createModule('/settings/fields', {
        label: FIELD_LABEL,
        data_type: 'string',
        entity: 'item',
  ...
  console.log('')
  console.log('--- .env ---')
  console.log(`ZOHO_CUSTOM_FIELD_ITEM_MIN_PURCHASE_ID=${fid}`)
  console.log('')

  const raw = readFileSync(envPath, 'utf8')
  writeFileSync(envPath, upsertEnvLine(raw, 'ZOHO_CUSTOM_FIELD_ITEM_MIN_PURCHASE_ID', fid), 'utf8')
